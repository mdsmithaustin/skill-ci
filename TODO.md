...
- `tools/check-pii.py` is a fourth verbatim copy from mds-pstack. It shares the fate of the other checkers: when mds-pstack consumes this repository's workflow, its copies and its `lint.yml` steps for them are deleted in the same change.

- `check-skill-content.py` requires every bolded skill name on a line containing the word "skill" to resolve under the local skills root. A repository whose skills legitimately reference skills that live elsewhere has no way to declare them. mds-pstack solved the adjacent problem with a `--foreign-file` allowlist in a separate script, which this checker does not have. The first consumer hit this once and the reference turned out to be genuinely miswritten, so no flag was invented. A repository with real cross-repository references needs the allowlist added in mds-pstack first and re-copied here.
