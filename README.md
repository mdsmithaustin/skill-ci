...
`skill-run` refuses to start when the readiness audit reports a blocker. Set `AGENTS` to limit which harnesses run, `RUNS` for repetitions, `MODEL` and `CODEX_MODEL` for the answering models and `JUDGE_MODEL` for the judge, `JUDGE_RUNS` for how many times each judge task repeats before the verdicts are merged, `TIMEOUT` for the per-run ceiling, `OUT` for the output directory, and `CODEX_CMD` to override the Codex command prefix (the configured default model on this host is rejected by the current codex-cli, so a working prefix names a model explicitly). This repository's own `mise.toml` includes the same task file and adds `test`, which runs the unit tests.
